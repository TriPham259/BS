...
public class OperatingSystem {
	...
	public synchronized int read(int pid, int virtAdr) {
		...
		if (pte == null) {
			// Seite nicht vorhanden:
			testOut(String.format("OS: read %d +++ Seitennr.: %d in Seitentabelle nicht vorhanden", pid,
					virtualPageNum));
			return -1;
		} else {
			// Seite vorhanden: Seite valid (im RAM)?
		...
		testOut("os: read " + pid + " +++ item: " + physRAM.get(realAddressOfItem) + " erfolgreich in virt. adresse " + virtAdr
